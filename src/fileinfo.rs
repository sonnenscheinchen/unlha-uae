...
const EXT_HEADER_DIR_ID: u8 = 0x02;
const EXT_HEADER_FILENAME_ID: u8 = 0x01;
const EXT_HEADER_LV2_COMMENT_ID: u8 = 0x71; // undocumented!
...
fn parse_level1(header: &LhaHeader) -> Result<FileInfo> {
    ...
    let comment = split.next();
    let mut path_components = vec![];
    if let Some(dir) = header.iter_extra().find(|e| e[0] == EXT_HEADER_DIR_ID) {
...
fn parse_level2(header: &LhaHeader) -> Result<FileInfo> {
    ...
    if let Some(name) = header.iter_extra().find(|e| e[0] == EXT_HEADER_FILENAME_ID) {
    ...
    if let Some(c) = header.iter_extra().find(|e| e[0] == EXT_HEADER_LV2_COMMENT_ID) {
        comment = Some(&c[1..]);
    }
